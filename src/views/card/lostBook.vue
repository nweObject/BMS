    ...
    <el-table-column label="操作">
      <template slot-scope="scope">
        <el-button
          size="mini"
          type="warning"
          @click="review(scope.row.id)" v-if="scope.row.status == 0">审核</el-button>
      <el-button
          size="mini"
          type="danger"
          @click="deleteBook(scope.row.bookId, scope.row.id)" v-if="scope.row.status == 1">下架</el-button>
      </template>
    </el-table-column>
  </el-table>
</div>
